...
# =======================================================
# LOGIN, PERMISSÕES E AUDITORIA
# =======================================================
USERS_FILE = "usuarios.json"
AUDIT_FILE = "auditoria.csv"
AUDIT_CLEAR_PASSWORD = "limpar123"
...
def _usuarios_padrao():
    usuarios = {
        ...
        "user": {
            "nome": "Usuário Padrão",
            "senha_hash": _hash_senha("user123"),
            "perfil": "visual",
            "ativo": True,
            "projetos": ["Todos"]
        }
...
def autenticar(usuario, senha):
    ...
    if not dados.get("ativo", True):
        return None
    if dados.get("senha_hash") == _hash_senha(senha):
        return dados
    return None

def registrar_auditoria(acao, detalhe=""):
    usuario = st.session_state.get("usuario", "não logado")
    ...
    if os.path.exists(AUDIT_FILE):
        registro.to_csv(AUDIT_FILE, mode="a", header=False, index=False, encoding="utf-8-sig")
    else:
        registro.to_csv(AUDIT_FILE, index=False, encoding="utf-8-sig")

def carregar_auditoria():
    if os.path.exists(AUDIT_FILE):
        try:
            return pd.read_csv(AUDIT_FILE, encoding="utf-8-sig")
        except Exception:
            pass
    return pd.DataFrame(columns=["Data/Hora", "Usuário", "Perfil", "Ação", "Detalhe"])
...
def tela_login():
    ...
    <div class="login-box">
        <div class="login-logo">GNO</div>
        <div class="login-sub">GRUPO NEW ORDER</div>
        <div class="login-title">Gestão de Projetos</div>
        <div class="login-caption">Acesso seguro ao painel corporativo</div>
    </div>
    ...
    if entrar:
        ...
        if dados:
            ...
            st.session_state.perfil = dados.get("perfil", "visual")
            st.session_state.projetos_permitidos = dados.get("projetos", ["Todos"])
            registrar_auditoria("LOGIN", f"Usuário {usuario} entrou no sistema")
            st.rerun()
        else:
...
PERFIL_ADMIN = st.session_state.get("perfil") == "admin"
PAGINAS_RESTRITAS_ADMIN = ["Cadastrar Projeto", "Editar Planilha", "Importar Excel"]

def aplicar_permissao_projetos(df):
    if PERFIL_ADMIN:
        return df
    if "Projeto" not in df.columns:
        return df

    projetos_permitidos = st.session_state.get("projetos_permitidos", ["Todos"])
    if "Todos" in projetos_permitidos:
        return df

    return df[df["Projeto"].astype(str).isin(projetos_permitidos)]


ARQUIVO_DADOS = "projetos.xlsx"
DB_FILE = "projetos.db"
TABELA_PROJETOS = "projetos"
...
def carregar_dados():
    ...
    Se o banco não existir, importa do Excel antigo uma única vez.
    Se não existir Excel, inicia vazio — sem voltar para dados de exemplo.
    """
    try:
        if os.path.exists(DB_FILE):
            with sqlite3.connect(DB_FILE) as conn:
                try:
                    df = pd.read_sql_query(f"SELECT * FROM {TABELA_PROJETOS}", conn)
        ...
        if os.path.exists(ARQUIVO_DADOS):
            df_excel = pd.read_excel(ARQUIVO_DADOS)
            for coluna in COLUNAS:
                if coluna not in df_excel.columns:
                    df_excel[coluna] = ""
            df_excel = tratar_datas(df_excel[COLUNAS])
            salvar_dados(df_excel)
            return df_excel
        ...
        st.error(f"Erro ao carregar dados persistentes: {erro}")
        return pd.DataFrame(columns=COLUNAS)


def salvar_dados(df):
    """
    Salva permanentemente em SQLite e gera Excel de backup.
    """
    try:
        df_salvar = df.copy()
        ...
        df_salvar = df_salvar[COLUNAS]

        for coluna_data in ["Data Início", "Data Fim"]:
            if coluna_data in df_salvar.columns:
                df_salvar[coluna_data] = pd.to_datetime(df_salvar[coluna_data], errors="coerce").dt.strftime("%Y-%m-%d")

        with sqlite3.connect(DB_FILE) as conn:
            df_salvar.to_sql(TABELA_PROJETOS, conn, if_exists="replace", index=False)

        df_salvar.to_excel(ARQUIVO_DADOS, index=False)

    except Exception as erro:
        st.error(f"Erro ao salvar dados no banco: {erro}")
...
# salvar_dados duplicado removido
...
def atualizar_status_automatico(df):
    ...
    for idx, row in df.iterrows():
        ...
        elif pd.notna(row["Data Fim"]) and pd.Timestamp(row["Data Fim"]) < hoje and row["Status"] != "Concluída":
...
def resumo_projetos(df):
    ...
    agrupado["Performance"] = pd.to_numeric(agrupado["Performance"], errors="coerce").fillna(0).round(0).astype(int)
    agrupado["Payback_Medio"] = pd.to_numeric(agrupado["Payback_Medio"], errors="coerce").fillna(0).round(1)
    return agrupado
...
def grafico_gantt(df):
    ...
    if "Etapa" not in gantt.columns:
        gantt["Etapa"] = ""
    if "Tarefa" not in gantt.columns:
        gantt["Tarefa"] = ""
    if gantt.empty:
        return None
    gantt["Linha"] = gantt["Etapa"].fillna("").astype(str) + " - " + gantt["Tarefa"].fillna("").astype(str)
    fig = px.timeline(
        gantt,
        x_start="Data Início",
        x_end="Data Fim",
...
with st.sidebar:
    st.caption(f"Usuário: {st.session_state.get('nome_usuario', '')}")
    st.caption(f"Perfil: {'Administrador' if PERFIL_ADMIN else 'Somente visualização'}")
    if st.button("Sair"):
        registrar_auditoria("LOGOUT", "Usuário saiu do sistema")
    ...
    menu_map = {
        ...
        "✎  Editar Planilha": "Editar Planilha",
    ...
    if (not PERFIL_ADMIN) and (pagina in PAGINAS_RESTRITAS_ADMIN or pagina in ["Usuários", "Auditoria"]):
        st.warning("🔒 Acesso restrito. Seu perfil permite somente visualização.")
        pagina = "Dashboard"
...
df = aplicar_permissao_projetos(st.session_state.df_projetos.copy())
...
if pagina == "Projetos":
    ...
    if not PERFIL_ADMIN:
        st.error("🔒 Apenas administradores podem alterar dados.")  # Cadastrar Projeto
        st.stop()
    ...
    with st.form("form_cadastro"):
        ...
        if st.form_submit_button("Salvar"):
            if not projeto or not tarefa:
                ...
                registrar_auditoria("CADASTRO DE PROJETO", f"Projeto: {projeto} | Tarefa: {tarefa} | Responsável: {responsavel}")
                st.success("Registro salvo com sucesso!")


elif pagina == "Atualizar Projeto":
    if not PERFIL_ADMIN:
        st.error("🔒 Apenas administradores podem atualizar projetos.")
        st.stop()

    st.title("Atualizar Projeto")
    st.caption("Filtre um projeto, altere informações existentes e adicione novas tarefas/etapas.")

    df_base = st.session_state.df_projetos.copy()

    if df_base.empty or "Projeto" not in df_base.columns:
        st.warning("Nenhum projeto cadastrado.")
    else:
        projetos_lista = sorted(df_base["Projeto"].dropna().astype(str).unique().tolist())
        projeto_selecionado = st.selectbox("Selecione o projeto para atualizar", projetos_lista)

        df_projeto = df_base[df_base["Projeto"].astype(str) == projeto_selecionado].copy()

        st.subheader("Resumo do Projeto")

        col1, col2, col3, col4 = st.columns(4)
        with col1:
            st.metric("Tarefas/Etapas", len(df_projeto))
        with col2:
            if "Percentual Concluído" in df_projeto.columns:
                st.metric("Progresso Médio", f"{df_projeto['Percentual Concluído'].mean():.1f}%")
        with col3:
            if "Budget" in df_projeto.columns:
                st.metric("Budget Total", f"R$ {df_projeto['Budget'].sum():,.2f}".replace(",", "X").replace(".", ",").replace("X", "."))
        with col4:
            if "Payback" in df_projeto.columns:
                st.metric("Payback Médio", f"{df_projeto['Payback'].mean():.1f} meses")

        st.divider()

        st.subheader("Editar informações do projeto")

        indices_projeto = df_projeto.index.tolist()

        df_editado = st.data_editor(
            df_projeto,
            num_rows="dynamic",
            use_container_width=True,
            key="editor_atualizar_projeto"
        )
        ...
        with col_salvar:
            if st.button("Salvar alterações do projeto", type="primary"):
                df_atualizado = st.session_state.df_projetos.copy()

                # remove linhas antigas do projeto
                df_atualizado = df_atualizado[df_atualizado["Projeto"].astype(str) != projeto_selecionado]

                # adiciona versão editada
                df_atualizado = pd.concat([df_atualizado, df_editado], ignore_index=True)

                st.session_state.df_projetos = df_atualizado
                salvar_dados(df_atualizado)
                registrar_auditoria(
                    "ATUALIZAÇÃO DE PROJETO",
                    f"Projeto atualizado: {projeto_selecionado} | Registros: {len(df_editado)}"
                )
        ...
        st.subheader("Adicionar nova tarefa/etapa neste projeto")

        with st.form("form_adicionar_tarefa_projeto"):
            c1, c2 = st.columns(2)

            with c1:
                departamento = st.text_input(
                    "Departamento",
                    value=str(df_projeto["Departamento"].dropna().iloc[0]) if "Departamento" in df_projeto.columns and not df_projeto["Departamento"].dropna().empty else ""
                )
                responsavel = st.text_input(
                    "Responsável",
                    value=str(df_projeto["Responsável"].dropna().iloc[0]) if "Responsável" in df_projeto.columns and not df_projeto["Responsável"].dropna().empty else ""
                )
                fornecedor = st.text_input(
                    "Fornecedor",
                    value=str(df_projeto["Fornecedor"].dropna().iloc[0]) if "Fornecedor" in df_projeto.columns and not df_projeto["Fornecedor"].dropna().empty else ""
                )
                etapa = st.text_input("Etapa")
                tarefa = st.text_input("Tarefa")
            ...
            with c2:
                ...
                status = st.selectbox("Status", STATUS_OPCOES if "STATUS_OPCOES" in globals() else ["Pendente", "Em Andamento", "Concluída", "Atrasada"])
                prioridade = st.selectbox("Prioridade", PRIORIDADE_OPCOES if "PRIORIDADE_OPCOES" in globals() else ["Baixa", "Média", "Alta", "Crítica"])
                percentual = st.slider("Percentual Concluído", 0, 150, 0)

            necessidade = st.text_area(
                "Necessidade do Projeto",
                value=str(df_projeto["Necessidade do Projeto"].dropna().iloc[0]) if "Necessidade do Projeto" in df_projeto.columns and not df_projeto["Necessidade do Projeto"].dropna().empty else ""
            )
            solicitante = st.text_input(
                "Solicitante do Projeto",
                value=str(df_projeto["Solicitante do Projeto"].dropna().iloc[0]) if "Solicitante do Projeto" in df_projeto.columns and not df_projeto["Solicitante do Projeto"].dropna().empty else ""
            )
            ...
            with c4:
                payback = st.number_input("Payback em meses", min_value=0.0, step=1.0, format="%.1f")
            with c5:
                overall = st.selectbox("Overall Task Status", OVERALL_STATUS_OPCOES if "OVERALL_STATUS_OPCOES" in globals() else ["Dentro do Planejado", "Atenção", "Crítico", "Bloqueado", "Finalizado"])

            c6, c7, c8 = st.columns(3)
            with c6:
                fase_in = st.selectbox("Fase In", FASE_OPCOES if "FASE_OPCOES" in globals() else ["Não iniciado", "Em execução", "Concluído", "Não aplicável"])
            with c7:
                fase_out = st.selectbox("Fase Out", FASE_OPCOES if "FASE_OPCOES" in globals() else ["Não iniciado", "Em execução", "Concluído", "Não aplicável"])
            with c8:
                riscos = st.text_input("Riscos do Projeto")

            c9, c10 = st.columns(2)
            with c9:
                probabilidade = st.selectbox("Probabilidade do Risco", PROBABILIDADE_RISCO_OPCOES if "PROBABILIDADE_RISCO_OPCOES" in globals() else ["Baixa", "Média", "Alta"])
            with c10:
                impacto = st.selectbox("Impacto do Risco", IMPACTO_RISCO_OPCOES if "IMPACTO_RISCO_OPCOES" in globals() else ["Baixo", "Médio", "Alto"])

            observacoes = st.text_area("Observações")

            adicionar = st.form_submit_button("Adicionar tarefa ao projeto")

        if adicionar:
            novo_registro = {}

            for coluna in st.session_state.df_projetos.columns:
                novo_registro[coluna] = ""

            valores = {
                "Projeto": projeto_selecionado,
                "Necessidade do Projeto": necessidade,
                "Solicitante do Projeto": solicitante,
                "Departamento": departamento,
                "Responsável": responsavel,
                "Fornecedor": fornecedor,
                "Etapa": etapa,
                ...
                "Prioridade": prioridade,
                "Percentual Concluído": percentual,
                "Overall Task Status": overall,
                "Fase In": fase_in,
                "Fase Out": fase_out,
                "Budget": budget,
                "Payback": payback,
                "Riscos": riscos,
                "Probabilidade do Risco": probabilidade,
                "Impacto do Risco": impacto,
                "Nível do Risco": calcular_nivel_risco(probabilidade, impacto) if "calcular_nivel_risco" in globals() else "",
                "Observações": observacoes
            }

            for chave, valor in valores.items():
                if chave in novo_registro:
                    novo_registro[chave] = valor

            st.session_state.df_projetos = pd.concat(
                [st.session_state.df_projetos, pd.DataFrame([novo_registro])],
                ignore_index=True
            )

            salvar_dados(st.session_state.df_projetos)
            registrar_auditoria(
                "ADIÇÃO DE TAREFA NO PROJETO",
                f"Projeto: {projeto_selecionado} | Tarefa: {tarefa} | Responsável: {responsavel}"
            )
            st.success("Nova tarefa adicionada ao projeto.")
            st.rerun()

elif pagina == "Editar Planilha":
    if not PERFIL_ADMIN:
        st.error("🔒 Apenas administradores podem alterar dados.")  # Editar Planilha
        st.stop()
    ...
    if st.button("Salvar alterações"):
        ...
        salvar_dados(st.session_state.df_projetos)
        registrar_auditoria("EDIÇÃO DE PLANILHA", "Planilha de projetos alterada pela tela Editar Planilha")
        st.success("Planilha salva.")

elif pagina == "Importar Excel":
    if not PERFIL_ADMIN:
        st.error("🔒 Apenas administradores podem alterar dados.")  # Importar Excel
        st.stop()
    ...
    if arquivo:
        try:
            ...
            if st.button("Usar esta planilha"):
                ...
                registrar_auditoria("IMPORTAÇÃO DE EXCEL", "Planilha Excel importada/substituída no sistema")
    ...
    st.caption("Cadastre usuários, defina perfis e permissões por projeto.")

    usuarios = carregar_usuarios()
    projetos_disponiveis = sorted(st.session_state.df_projetos["Projeto"].dropna().astype(str).unique().tolist())
    ...
    for login, dados in usuarios.items():
        linhas.append({
            ...
            "Projetos Permitidos": ", ".join(dados.get("projetos", ["Todos"]))
        })
    ...
    with st.form("form_novo_usuario"):
        ...
        novo_perfil = st.selectbox("Perfil", ["visual", "admin"])
        projetos_permitidos = st.multiselect(
            "Projetos permitidos",
            ["Todos"] + projetos_disponiveis,
            default=["Todos"]
        )
        criar_usuario = st.form_submit_button("Criar usuário")
    ...
    if criar_usuario:
        if not novo_login or not nova_senha:
            ...
            usuarios[novo_login] = {
                "nome": novo_nome or novo_login,
                "senha_hash": _hash_senha(nova_senha),
                "perfil": novo_perfil,
                "ativo": True,
                "projetos": projetos_permitidos or ["Todos"]
            }
            salvar_usuarios(usuarios)
            registrar_auditoria("CRIAÇÃO DE USUÁRIO", f"Usuário criado: {novo_login} | Perfil: {novo_perfil}")
    ...
    usuario_edicao = st.selectbox("Selecionar usuário", list(usuarios.keys()))
    dados_usuario = usuarios[usuario_edicao]

    with st.form("form_editar_usuario"):
        nome_edit = st.text_input("Nome", value=dados_usuario.get("nome", ""))
        perfil_edit = st.selectbox(
            "Perfil",
            ["visual", "admin"],
            index=["visual", "admin"].index(dados_usuario.get("perfil", "visual"))
        )
        ativo_edit = st.checkbox("Usuário ativo", value=dados_usuario.get("ativo", True))
        projetos_edit = st.multiselect(
            "Projetos permitidos",
            ["Todos"] + projetos_disponiveis,
            default=dados_usuario.get("projetos", ["Todos"])
        )
        nova_senha_edit = st.text_input("Nova senha, se quiser alterar", type="password")
        salvar_usuario = st.form_submit_button("Salvar alterações")

    if salvar_usuario:
        usuarios[usuario_edicao]["nome"] = nome_edit
        usuarios[usuario_edicao]["perfil"] = perfil_edit
        usuarios[usuario_edicao]["ativo"] = ativo_edit
        usuarios[usuario_edicao]["projetos"] = projetos_edit or ["Todos"]

        if nova_senha_edit:
            usuarios[usuario_edicao]["senha_hash"] = _hash_senha(nova_senha_edit)

        salvar_usuarios(usuarios)
        registrar_auditoria("ALTERAÇÃO DE USUÁRIO", f"Usuário alterado: {usuario_edicao}")
    ...
    st.caption("Histórico de ações executadas no sistema: quem alterou, o que alterou e quando.")
    ...
    if auditoria.empty:
        ...
        with col_a1:
            usuarios_log = ["Todos"] + sorted(auditoria["Usuário"].dropna().astype(str).unique().tolist())
            filtro_usuario_log = st.selectbox("Filtrar por usuário", usuarios_log)

        with col_a2:
            acoes_log = ["Todas"] + sorted(auditoria["Ação"].dropna().astype(str).unique().tolist())
            filtro_acao_log = st.selectbox("Filtrar por ação", acoes_log)

        with col_a3:
            qtd_exibir = st.number_input("Quantidade de registros", min_value=10, max_value=5000, value=200, step=10)

        aud_filtrada = auditoria.copy()

        if filtro_usuario_log != "Todos":
            aud_filtrada = aud_filtrada[aud_filtrada["Usuário"] == filtro_usuario_log]

        if filtro_acao_log != "Todas":
            aud_filtrada = aud_filtrada[aud_filtrada["Ação"] == filtro_acao_log]

        aud_filtrada = aud_filtrada.tail(int(qtd_exibir)).sort_values("Data/Hora", ascending=False)

        c1, c2, c3 = st.columns(3)
        c1.metric("Registros", len(aud_filtrada))
        c2.metric("Usuários", aud_filtrada["Usuário"].nunique())
        c3.metric("Tipos de Ação", aud_filtrada["Ação"].nunique())

        st.dataframe(aud_filtrada, use_container_width=True, hide_index=True)

        st.download_button(
            "Baixar log de auditoria CSV",
            data=aud_filtrada.to_csv(index=False).encode("utf-8-sig"),
            file_name="log_auditoria.csv",
            mime="text/csv"
        )

        with st.expander("Limpar log de auditoria (restrito)"):
            senha_limpar = st.text_input("Senha para limpeza do log", type="password")
            confirmar_limpeza = st.button("Confirmar limpeza do log")

            if confirmar_limpeza:
                if senha_limpar == AUDIT_CLEAR_PASSWORD:
                    registrar_auditoria("LIMPEZA DE LOG", "Administrador limpou o log de auditoria")
                    pd.DataFrame(columns=["Data/Hora", "Usuário", "Perfil", "Ação", "Detalhe"]).to_csv(
                        AUDIT_FILE,
                        index=False,
                        encoding="utf-8-sig"
                    )
                    st.success("Log de auditoria limpo com sucesso.")
                    st.rerun()
                else:
    ...
    with col_b1:
        if os.path.exists(DB_FILE):
            with open(DB_FILE, "rb") as arquivo_db:
                st.download_button(
                    "Baixar banco de dados SQLite",
                    data=arquivo_db,
                    file_name="projetos.db",
                    mime="application/octet-stream"
                )

    with col_b2:
        if os.path.exists(ARQUIVO_DADOS):
            with open(ARQUIVO_DADOS, "rb") as arquivo_excel:
                st.download_button(
                    "Baixar backup Excel",
                    data=arquivo_excel,
                    file_name="projetos_backup.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                )

    st.info("Os dados agora são gravados em projetos.db. O Excel é apenas backup.")
    ...
    if alterar_senha:
        ...
        if not dados or dados.get("senha_hash") != _hash_senha(senha_atual):
            st.error("Senha atual incorreta.")
        elif not nova_senha or nova_senha != confirma_senha:
            st.error("A nova senha está vazia ou não confere.")
        else:
            usuarios[usuario_atual]["senha_hash"] = _hash_senha(nova_senha)
            salvar_usuarios(usuarios)
            registrar_auditoria("TROCA DE SENHA", f"Usuário alterou a própria senha: {usuario_atual}")
