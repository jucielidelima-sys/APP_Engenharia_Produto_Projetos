...
def _usuarios_padrao():
    ...
    with open(USERS_FILE, "w", encoding="utf-8") as arq:
...
def carregar_usuarios():
    ...
    try:
        with open(USERS_FILE, "r", encoding="utf-8") as arq:
...
def autenticar(usuario, senha):
    ...
    if not dados or not dados.get("ativo", True):
        return None
...
def tela_login():
    ...
    .login-logo {font-size: 48px; font-weight: 900; color: #f4511e;}
    .login-sub {font-size: 11px; letter-spacing: 3px; color: #f4511e; margin-bottom: 20px;}
    .login-title {font-size: 28px; font-weight: 900; color: #142331;}
    .login-caption {color: #667085; margin-bottom: 16px;}
    </style>
    <div class="login-box">
...
PAGINAS_ADMIN = ["Cadastrar Projeto", "Editar Planilha", "Importar Excel"]

def aplicar_permissao_projetos(df):
    if PERFIL_ADMIN or "Projeto" not in df.columns:
        return df
    projetos = st.session_state.get("projetos_permitidos", ["Todos"])
    if "Todos" in projetos:
        return df
    return df[df["Projeto"].astype(str).isin(projetos)]
...
with st.sidebar:
    ...
    if (not PERFIL_ADMIN) and (pagina in PAGINAS_ADMIN or pagina in ["Usuários", "Auditoria"]):
...
if pagina == "Projetos":
    ...
    if not PERFIL_ADMIN:
        st.error("🔒 Apenas administradores podem alterar dados.")
        st.stop()
    ...
    usuarios = carregar_usuarios()
    projetos = sorted(st.session_state.df_projetos["Projeto"].dropna().astype(str).unique().tolist())
    ...
    for login, dados in usuarios.items():
        linhas.append({
            ...
            "Projetos": ", ".join(dados.get("projetos", ["Todos"]))
        })
    ...
    with st.form("novo_usuario"):
        ...
        novos_projetos = st.multiselect("Projetos permitidos", ["Todos"] + projetos, default=["Todos"])
    ...
    if criar:
        if not novo_login or not nova_senha:
            ...
            usuarios[novo_login] = {
                ...
                "perfil": novo_perfil,
                "ativo": True,
                "projetos": novos_projetos or ["Todos"]
            }
    ...
    usuario_edit = st.selectbox("Usuário", list(usuarios.keys()))
    dados = usuarios[usuario_edit]

    with st.form("editar_usuario"):
        nome_edit = st.text_input("Nome", value=dados.get("nome", ""))
        perfil_edit = st.selectbox("Perfil", ["visual", "admin"], index=["visual", "admin"].index(dados.get("perfil", "visual")))
        ativo_edit = st.checkbox("Ativo", value=dados.get("ativo", True))
        projetos_edit = st.multiselect("Projetos permitidos", ["Todos"] + projetos, default=dados.get("projetos", ["Todos"]))
        senha_edit = st.text_input("Nova senha, opcional", type="password")
        salvar = st.form_submit_button("Salvar alterações")

    if salvar:
        usuarios[usuario_edit]["nome"] = nome_edit
        usuarios[usuario_edit]["perfil"] = perfil_edit
        usuarios[usuario_edit]["ativo"] = ativo_edit
        usuarios[usuario_edit]["projetos"] = projetos_edit or ["Todos"]
        if senha_edit:
            usuarios[usuario_edit]["senha_hash"] = _hash_senha(senha_edit)
        salvar_usuarios(usuarios)
        registrar_auditoria("ALTERAÇÃO DE USUÁRIO", f"Usuário alterado: {usuario_edit}")
...
with st.expander("Limpar log de auditoria (restrito)"):
    ...
    confirmar_limpeza = st.button("Confirmar limpeza")
    ...
    with st.form("trocar_senha"):
        ...
        trocar = st.form_submit_button("Alterar senha")

    if trocar:
        usuarios = carregar_usuarios()
        usuario = st.session_state.get("usuario")
        if usuarios[usuario]["senha_hash"] != _hash_senha(senha_atual):
            ...
            salvar_usuarios(usuarios)
            registrar_auditoria("TROCA DE SENHA", f"Usuário alterou a própria senha: {usuario}")
